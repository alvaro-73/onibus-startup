...
onibus: Ponto,
inicioSegmento = 0,
fimSegmento = rota.length - 2
) {
let menorDistancia = Infinity;
let melhorPonto = rota[inicioSegmento];
let melhorSegmento = inicioSegmento;
...
* Pega somente o trecho da geometria das ruas
* entre a origem/posição do ônibus e a próxima parada.
*
* IMPORTANTE:
* Não existe fallback para [inicio, destino].
* Se o ORS não devolver os way_points corretamente,
* não desenhamos uma linha reta sobre as casas.
  */
...
const inicioIndice =
wayPoints[indiceParada];

const destinoIndice =
wayPoints[indiceParada + 1];

if (
inicioIndice === undefined ||
destinoIndice === undefined
) {
return [];
}
...
* Calcula UMA rota contendo todas as paradas.
* O ORS retorna a geometria pelas ruas e os
* way_points indicam onde cada parada está nessa geometria.
  */
  useEffect(() => {
  let cancelado = false;
...
```
  ...
  try {
    ...
    if (
      ...
      console.error(
        "way_points inválidos:",
        pontosRota
      );

      throw new Error(
        "A rota não retornou os pontos das paradas corretamente."
      );
    }
...
* Quando o ônibus chega a uma parada,
* ela é concluída e a próxima passa a ser exibida.
  */
  useEffect(() => {
  if (
  !onibusPosicao ||
...
* Coloca o ícone do ônibus sobre a geometria
* da rua mais próxima.
  */
...
```
  ...
  >
    ...
    {/*
     * MOSTRA SOMENTE O TRAJETO DA PRÓXIMA PARADA.
     * A geometria vem diretamente do ORS,
     * então não é criada nenhuma linha reta entre casas.
     */}
